...
df_param = pd.read_excel(home+'/testing/test_indicators.xlsx', sheet_name='template')
...
df_B = pd.read_excel(home+'/testing/test_budget.xlsx', sheet_name='template_expenditure')
Bs = df_B.expenditure

df_B = pd.read_excel(home+'/testing/test_budget.xlsx', sheet_name='template_relation_table').dropna(axis=1, how='all')
B_dict = {}
for index, row in df_B.iterrows():
    B_dict[int(row[0]-1)] = [int(c-1) for c in row[df_B.columns[1::]].values if not np.isnan(c)]
    

df_A = pd.read_excel(home+'/testing/test_network.xlsx', sheet_name='template_network')
