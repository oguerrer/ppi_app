...
def calibrate(I0, IF, success_rates, A=None, R=None, qm=None, rl=None,  Bs=None, B_dict=None, 
              T=None, threshold=.7, parallel_processes=None, 
              verbose=False, low_precision_counts=50, increment=100):
    ...
    while np.mean(GoF_alpha)<threshold or np.mean(GoF_beta)<threshold:
        ...
        if verbose:
            print( 'Iteration:', counter, '.    Worst goodness of fit:', np.min([np.mean(GoF_alpha), np.mean(GoF_beta)]) )
